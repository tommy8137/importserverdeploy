...
setup_docker() {
    ...
    then
        ...
        echo ${VM_PASSWORD} | sudo -S apt-get update -y
        echo ${VM_PASSWORD} | sudo -S apt-get install apt-transport-https ca-certificates curl gnupg-agent software-properties-common -y
        echo ${VM_PASSWORD} | sudo -S curl -fsSL https://download.docker.com/linux/ubuntu/gpg | sudo apt-key add -
        echo ${VM_PASSWORD} | sudo -S add-apt-repository "deb [arch=amd64] https://download.docker.com/linux/ubuntu $(lsb_release -cs) stable"
        echo ${VM_PASSWORD} | sudo -S apt-get install docker-ce docker-ce-cli containerd.io -y
        echo "==================== Start docker ===================="
...
setup_ntpd() {
    ...
    if [ $? -eq 0 ]; then
        ...
        # echo ${VM_PASSWORD} | sudo -S apt-get list installed ntp -y
        echo ${VM_PASSWORD} | sudo -S apt-get install ntp -y
        echo ${VM_PASSWORD} | sudo -S systemctl start ntp
        echo ${VM_PASSWORD} | sudo -S systemctl enable ntp
    fi
